...
export default function HomePage() {
  ...
  // Track analysis score for dynamic corridors to update map color
  const { data: analysis } = useCorridorAnalysis(selectedCorridorId, selectedCorridor);

  // Update dynamic polygon color when score arrives
  useEffect(() => {
    if (
      selectedCorridor &&
      !selectedCorridor.isPreset &&
      selectedCorridor.polygon &&
      analysis
    ) {
      mapRef.current?.addDynamicPolygon(
        selectedCorridor.polygon,
        selectedCorridor.name,
        analysis.score
      );
    }
  }, [analysis, selectedCorridor]);
